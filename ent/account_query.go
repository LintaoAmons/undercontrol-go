...
type AccountQuery struct {
	...
	withAccounthistory *AccountHistoryQuery
...
// QueryAccounthistory chains the current query on the "accounthistory" edge.
func (aq *AccountQuery) QueryAccounthistory() *AccountHistoryQuery {
	query := (&AccountHistoryClient{config: aq.config}).Query()
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withAccounthistory: aq.withAccounthistory.Clone(),
...
// WithAccounthistory tells the query-builder to eager-load the nodes that are connected to
// the "accounthistory" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithAccounthistory(opts ...func(*AccountHistoryQuery)) *AccountQuery {
	query := (&AccountHistoryClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withAccounthistory = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withAccounthistory; query != nil {
		if err := aq.loadAccounthistory(ctx, query, nodes,
			func(n *Account) { n.Edges.Accounthistory = []*AccountHistory{} },
			func(n *Account, e *AccountHistory) { n.Edges.Accounthistory = append(n.Edges.Accounthistory, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadAccounthistory(ctx context.Context, query *AccountHistoryQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountHistory)) error {
	...
	query.withFKs = true
	query.Where(predicate.AccountHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.AccounthistoryColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_accounthistory
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_accounthistory" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_accounthistory" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
